...
async function loadTradeLog() {
  try {
    ...
    // Update Fear & Greed from most recent decision that has it
    const fgEl = document.getElementById("fear-greed");
    if (fgEl) {
      const recent = logs.find(l => l.decision?.signals_summary?.includes("F&G") ||
        (l.decision?.signals_summary && l.indicators?.regime));
      if (recent?.decision?.signals_summary) {
        const match = recent.decision.signals_summary.match(/F&G[:\s]+(\d+)/i);
        if (match) fgEl.textContent = match[1] + "/100";
      }
    }
...
function buildLogEntry(log) {
  ...
  const regime  = log.indicators?.regime ? `<span class="regime-badge regime-${log.indicators.regime}">${log.indicators.regime.replace("_", " ")}</span> ` : "";
  const hist    = log.history ? `<div class="log-history">
    <span class="${log.history.ltTrend === 'BULL' ? 'positive' : 'negative'}">${log.history.ltTrend}</span> macro ·
    30d: <span class="${log.history.change30d >= 0 ? 'positive' : 'negative'}">${log.history.change30d > 0 ? '+' : ''}${log.history.change30d}%</span> ·
    from ATH: ${log.history.distFromHigh}% ·
    ${log.history.nearSupport ? '<span class="positive">near support</span>' : log.history.nearResistance ? '<span class="negative">near resistance</span>' : 'no key level'}
  </div>` : "";
  ...
  return `<div class="log-entry ${cls}">
    <div class="log-header">
      <span class="log-symbol">${log.symbol} ${typeBadge}${regime}</span>
...
async function loadSchedulerStatus() {
  try {
    const { running, active, circuitBreaker } = await api("/api/scheduler/status");
    const statusEl = document.getElementById("scheduler-status");

    // Circuit breaker banner
    const cbBanner = document.getElementById("circuit-breaker-banner");
    if (cbBanner) {
      cbBanner.classList.toggle("hidden", !circuitBreaker?.active);
    }
